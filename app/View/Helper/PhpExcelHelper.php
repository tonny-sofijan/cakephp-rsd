...
class PhpExcelHelper extends AppHelper {
	...
	/* font formatting can be bold, italic, underline, strike, superScript, subScript */

	public function setFontFormatting($offset, $style = 'bold') {
		switch ($style) {
			case 'bold':
				$this->xls->getActiveSheet()->getStyle($offset)->getFont()->setBold(true);
				break;
			case 'italic':
				$this->xls->getActiveSheet()->getStyle($offset)->getFont()->setItalic(true);
				break;
			case 'underline':
				break;
				$this->xls->getActiveSheet()->getStyle($offset)->getFont()->setUnderline(true);
			case 'strike':
				break;
				$this->xls->getActiveSheet()->getStyle($offset)->getFont()->setStrike(true);
			case 'superScript':
				break;
				$this->xls->getActiveSheet()->getStyle($offset)->getFont()->setSuperScript(true);
			case 'subScript':
				break;
				$this->xls->getActiveSheet()->getStyle($offset)->getFont()->setSubScript(true);
		}
	}

	/* Alignment can be left, center, right, justify */

	public function setAlignment($offset, $alignment) {
		switch (strtolower($alignment)) {
			case 'left':
				$this->xls->getActiveSheet()->getStyle($offset)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
				break;
			case 'right':
				$this->xls->getActiveSheet()->getStyle($offset)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
				break;
			case 'center':
				$this->xls->getActiveSheet()->getStyle($offset)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				break;
			case 'justify':
				$this->xls->getActiveSheet()->getStyle($offset)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
				break;
		}
	...
	public function setNumberFormat($offset) {
		$this->xls->getActiveSheet()->getStyle($offset)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
	}

	/**
	 * Set Border 
	 * $range = from:to. eg. A1:G1
	 * $borderStyle = allborders, bottom, top, left, right
	 * $thickness = thin, medium, thick
	 */
	public function setBorder($range, $borderStyle, $thickness) {
		$border = $style = '';
		switch ($thickness) {
			case '0':
				$styleArray = array('borders' => array(strtolower($borderStyle) => array('style' => PHPExcel_Style_Border::BORDER_NONE)));
				break;
			case '1':
				$styleArray = array('borders' => array(strtolower($borderStyle) => array('style' => PHPExcel_Style_Border::BORDER_HAIR)));
				break;
			case '2':
				$styleArray = array('borders' => array(strtolower($borderStyle) => array('style' => PHPExcel_Style_Border::BORDER_THIN)));
				break;
			case '3':
				$styleArray = array('borders' => array(strtolower($borderStyle) => array('style' => PHPExcel_Style_Border::BORDER_MEDIUM)));
				break;
			case '4':
				$styleArray = array('borders' => array(strtolower($borderStyle) => array('style' => PHPExcel_Style_Border::BORDER_THICK)));
				break;
		}

		$this->xls->getActiveSheet()->getStyle($range)->applyFromArray($styleArray);

		unset($style, $styleArray, $border);
	}
	...
	/**
	 * Write array of data to actual row starting from column defined from start to end
	 * Start and end is column in excel (alphabetic)
	 */
	public function addMergedData($data, $start, $end, $alignment = 'center') {
		$offset = PHPExcel_Cell::columnIndexFromString($start);
		foreach ($data as $d) {
			$this->xls->getActiveSheet()->setCellValueByColumnAndRow($start, $this->row, $d);
			switch (strtolower($alignment)) {
				case 'left':
					$this->xls->getActiveSheet()->getStyle($start . $this->row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
					break;
				case 'center':
					$this->xls->getActiveSheet()->getStyle($start . $this->row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					break;
				case 'right':
					$this->xls->getActiveSheet()->getStyle($start . $this->row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
					break;
				case 'justify':
					$this->xls->getActiveSheet()->getStyle($start . $this->row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY);
					break;
				default:
					$this->xls->getActiveSheet()->getStyle($start . $this->row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					break;
			...
			#$this->xls->getActiveSheet()->getStyle($start . $this->row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->xls->getActiveSheet()->mergeCells($start . $this->row . ':' . $end . $this->row);
			$this->row++;
		}
	}
